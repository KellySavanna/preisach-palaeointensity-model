...
# extract files from the current directory
...
def find_files(X):
    ...
    ----------
    X : dict
        Dictionary to store sample file paths and metadata.
...
def proccess_all(X):
    """
    Process a FORC dataset stored in dictionary *X* by parsing the measurement file,
    extracting magnetic parameters, assigning metadata, and converting units if needed.

    Parameters
    ----------
    X : dict
        Dictionary containing at least the key 'fn', which is a filepath to a FORC file.

    Returns
    -------
    dict
        The updated dictionary `X` with parsed measurement values, metadata, and units.
    """

    # Dictionary styling placeholder (may be used for UI settings or metadata)
    style = {'description_width': 'initial'}

    # Extract the filename for metadata purposes
    fn = X['fn']

    # Parse the sample name from the filename (take last part of path and remove extension)
    sample = fn.split('/')[-1]
    sample = sample.split('.')
    X["sample"] = sample

    # Assign default measurement unit system
    unit = 'SI'
    X["unit"] = unit

    # Parse the magnetic FORC measurement file and extract measurement arrays
    H, Hr, M, Fk, Fj, dH = parse_measurements(X["fn"])

    # Store extracted measurement data in dictionary
    X["H"] = H      # Applied field
    X["Hr"] = Hr    # Reversal field
    X["M"] = M      # Magnetization
    X["dH"] = dH    # Field increment resolution
    X["Fk"] = Fk    # FORC kernel values
    X["Fj"] = Fj    # FORC distribution values

    # Assign magnetic bounds (useful for plotting or normalization)
    X["Hc1"] = 0.0        # Lower coercivity bound
    X["Hc2"] = max(H)     # Upper coercivity bound
    X["Hb1"] = max(Hr)    # Max bias field
    X["Hb2"] = min(Hr)    # Min bias field
...
def parse_measurements(file):
    """
    Parse FORC (First-Order Reversal Curve) measurement data from a file and extract
    magnetic field values, reversal fields, magnetization values, and associated
    measurement indices. The function detects FORC boundaries, removes placeholder
    and NaN values, and returns the processed dataset along with the calculated
    mean field spacing.

    Parameters
    ----------
    file : str
        Path to the FORC measurement file to be processed.

    Returns
    -------
    tuple
        H  : numpy.ndarray
             Applied field values (concatenated across all FORCs)
        Hr : numpy.ndarray
             Reversal field values for each measurement
        M  : numpy.ndarray
             Magnetization values
        Fk : numpy.ndarray
             FORC index number identifier for each datapoint
        Fj : numpy.ndarray
             Measurement index within each FORC
        dH : float
             Estimated mean field spacing
    """

    # Dummy placeholder value used to identify blank lines or separators
    dum = -9999.99 
    
    # Preallocate large arrays to hold measurement values (later trimmed)
    N0 = int(1E6) 
    H0 = np.zeros(N0) * np.nan  # Field storage initialized as NaN
    M0 = np.zeros(N0) * np.nan  # Magnetization storage initialized as NaN

    # First entry is set to dummy placeholder
    H0[0] = dum
    M0[0] = dum

    # Counter controlling insertion of extracted values
    count = 0 

    # Open file using expected encoding
    with cd.open(file, "r", encoding='latin9') as fp: 
        
        # Iterate over only valid data lines
        for line in find_data_lines(fp): 
            count = count + 1  # Increment counter

            # Check for presence of comma (indicates valid data structure)
            idx = line.find(',') 

            if idx > -1:  # Valid formatted line
                H0[count] = float(line[0:idx])  # Extract first column (field)
                line = line[idx+1:]  # Strip first field, keep rest of line
                
                idx = line.find(',')  # Look for second column boundary

                if idx > -1:
                    # Assign magnetization from second column
                    M0[count] = float(line[0:idx])
                else:
                    # No second comma: entire line is magnetization
                    M0[count] = float(line)

            else:
                # Blank or invalid line → assign dummy values
                H0[count] = dum
                M0[count] = dum
                
    # Identify index where real data begins (first non-dummy entry)
    idx_start = np.argmax(H0 != dum)

    # Remove leading dummy values but retain the first placeholder for indexing
    M0 = M0[idx_start-1:-1]
    ...
    # Identify indices separating individual FORCs (blank lines encoded as dummy values)
    idxSAT = np.array(np.where(np.isin(H0, dum)))
    idxSAT = np.ndarray.squeeze(idxSAT)

    # Start and end indices for each FORC curve
    idxSTART = idxSAT[0::1] + 1
    idxEND = idxSAT[1::1] - 1

    # ------- Extract the first FORC -------
    M = M0[idxSTART[0]:idxEND[0]]
    H = H0[idxSTART[0]:idxEND[0]]

    Hr = np.ones(idxEND[0] - idxSTART[0]) * H0[idxSTART[0]]
    Fk = np.ones(idxEND[0] - idxSTART[0])
    Fj = np.arange(1, 1 + idxEND[0] - idxSTART[0])

    # ------- Extract remaining FORCs -------
    for i in range(1, idxSTART.size - 1):
        M = np.concatenate((M, M0[idxSTART[i]:idxEND[i]]))
        H = np.concatenate((H, H0[idxSTART[i]:idxEND[i]]))
        Hr = np.concatenate((Hr, np.ones(idxEND[i] - idxSTART[i]) * H0[idxSTART[i]]))
        Fk = np.concatenate((Fk, np.ones(idxEND[i] - idxSTART[i]) + i))
        Fj = np.concatenate((Fj, np.arange(1, 1 + idxEND[i] - idxSTART[i])))
        mi = i  # Tracks last processed FORC index

    # Ensure SI units (conversion skipped unless explicitly CGS)
    unit = 'SI'
    ...
    # Compute mean field spacing based on last FORC
    dH = (-H0[idxSTART[i]] + H0[idxEND[i]]) / (idxEND[i] + 1 - idxSTART[i])

    # Return parsed FORC dataset
    return H, Hr, M, Fk, Fj, dH
...
# ===== creating and plotting forcs (copy from original) =====
# note - some functions / steps may be redunant here (need to check) + remove commented out sections   
...
def rotate_FORC(X):
    """
    Rotate structured FORC arrays from (H, Hr) coordinates to standard
    FORC coordinates (Hc, Hu), where:
        Hc = (H - Hr)/2  → coercivity axis
        Hu = (H + Hr)/2  → interaction/bias field axis

    This is a standard transformation used in FORC diagrams for plotting
    and interpretation of magnetic interactions.
    ...
    -------
    dict
        Updated dictionary with rotated axes:
        - 'Hc' : coercivity axis
        - 'Hu' : bias/interaction field axis
    """

    # Extract structured field arrays
    H_A = X['H_A']
    Hr_A = X['Hr_A']

    # Perform rotation to standard FORC coordinates
    Hc = (H_A - Hr_A) / 2.  # coercivity axis
    Hu = (H_A + Hr_A) / 2.  # interaction/bias axis
...
def half_max_test(fwHu_c, fwRho_c, ym):
    """
    Calculate the Full Width at Half Maximum (FWHM) for a given peak in a dataset.
    The function finds the positions on both sides of the peak where the function
    value reaches half of its maximum, and interpolates between points to improve
    accuracy.

    Parameters
    ----------
    fwHu_c : np.ndarray
        Array of x-axis values (e.g., Hu coordinates) corresponding to the dataset.
    fwRho_c : np.ndarray
        Array of y-axis values (e.g., rho values) representing the peak.
    ym : float
        Maximum value of the peak to determine the half-maximum (peak height).
    ...
    tuple
        ...
        r0 : float
            Left-side position at half maximum.
        r1 : float
            Right-side position at half maximum.
    """

    # Find index of the peak maximum in the dataset
    ...
    # Initialize right-side index search
    b = L + 1  

    # Search forward for the first point below half maximum
    while (b < len(fwRho_c)):
        if(fwRho_c[b] < half_ym):       
            break
        b = b + 1  

    # Interpolate to estimate more precise position at half maximum
    top = fwRho_c[b-1] - fwRho_c[b]
    bot = fwHu_c[b-1] - fwHu_c[b]
    mo_test = top / bot
    r0 = fwHu_c[b] + ((half_ym - fwRho_c[b]) / mo_test)

    # Initialize left-side index search
    u = L - 1
    r1 = -2

    # Search backward for the first point below half maximum
    while u > 0:
    ...
    # Interpolate left-side position if necessary
    if r1 < 0:
        r1 = -r0
    else:
    ...
    # Calculate FWHM as distance between left and right half-maximum points
    fwhm = r1 - r0
...
def norm_rho_all(X):
    """
    Normalize each 2D FORC distribution (rho) by its maximum value for all smoothing factors.

    This function:
      - Copies the original Rho array to avoid modifying the input
      - Normalizes each 2D slice of Rho along the first axis (smoothing factors) by its maximum value
      - Stores the normalized Rho and maximum values back in the dictionary `X`

    Parameters
    ----------
    X : dict
        Dictionary containing:
        - 'rho': 3D array of FORC distributions (shape: [SF, no_FORC, max_FORC_len])

    Returns
    -------
    dict
        Updated dictionary `X` containing:
        - 'rho_n': normalized Rho array
        - 'max_Rho': array of maximum Rho values for each SF
    """

    Rho = X['rho']  # Original FORC distribution array
    Rho_n = np.copy(Rho)  # Create a copy to avoid modifying original data
    a, x, y = np.shape(Rho)  # a: number of smoothing factors, x: no_FORC, y: max_FORC_len

    X['max_Rho'] = np.zeros((a))  # Initialize array to store max value for each SF

    k = 0
    for k in range(2, a):  # Start normalization from SF=2 onward
        i = 0
        j = 0
        max_Rho = np.nanmax(Rho_n[k])  # Maximum value of current SF slice

        # Normalize each element in the 2D slice by the maximum value
        for i in range(x):
            for j in range(y):
                Rho_n[k][i][j] = Rho[k][i][j] / max_Rho

        X['rho_n'] = Rho_n  # Store normalized array in dictionary
        X['max_Rho'][k] = max_Rho  # Store maximum value for this SF
        k += 1

    return(X)

  
def plot_sample_FORC(x, y, z, SF, sample_name, hcmaxa, himina):
    """
    Neatly plot a FORC (First-Order Reversal Curve) diagram for a sample,
    including a dashed x=y line for reference.
    """

    # Copy Rho data to avoid modifying original
    zn = np.copy(z)
    ...
    # Get colormap and min/max values using FORCinel conventions
    ...
    # Save figure
    filename = os.path.join(f'{sample_name}_nrf',
...
def inter_FORC_for_plot(X, SF):
    """
    Interpolate normalized FORC rho data onto a regular grid for plotting.

    This function takes the FORC data for a given smoothing factor (SF),
    flattens the H_A, Hr_A, and normalized Rho arrays, removes NaN values,
    and interpolates the data onto a regular 2D grid suitable for plotting.

    Parameters
    ----------
    X : dict
        Dictionary containing FORC data, including 'H_A', 'Hr_A', and 'rho_n'.
    SF : int
        The smoothing factor (index) of the Rho array to interpolate.
    ...
    X : dict
        Updated dictionary containing the interpolated grid arrays:
        - 'xia': x-coordinates of the target grid (H_A)
        - 'yia': y-coordinates of the target grid (Hr_A)
        - 'zia': interpolated Rho values on the target grid for the given SF
        - 'hcmaxa': maximum H_A value in mT for plotting limits
        - 'himina': minimum Hr_A value in mT for plotting limits
    """

    # Flatten the H_A, Hr_A, and normalized Rho arrays for interpolation
    Ha_f = X['H_A'].flatten() 
    Hr_f = X['Hr_A'].flatten()
    Rho_f = X['rho_n'][SF].flatten() 

    # Combine arrays into a dictionary for simultaneous NaN removal
    data = {"Ha_f": Ha_f, "Hr_f": Hr_f, "Rho_f": Rho_f}
    ...
    # Apply the mask to remove NaN values from all arrays
    cleaned_data = {key: data[key][mask] for key in data}
    Ha_f = cleaned_data['Ha_f']
    Hr_f = cleaned_data['Hr_f']
    Rho_f = cleaned_data['Rho_f']

    # Define grid steps for interpolation (fine grid for smooth plotting)
    step_xia = np.nanmax(X['H_A']) / 500.
    step_yia = (np.nanmax(X['Hr_A']) - np.nanmin(X['Hr_A'])) / 500.

    # Define the target grid for plotting
    xi = np.arange(0, np.nanmax(X['H_A']), step_xia)
    yi = np.arange(np.nanmin(X['Hr_A']), np.nanmax(X['Hr_A']), step_yia)
    xia, yia = np.meshgrid(xi, yi) 

    # Interpolate the Rho data onto the regular grid using cubic interpolation
    zia = griddata((Ha_f, Hr_f), Rho_f, (xia, yia), method='cubic') 

    # Save the interpolated grids and plotting limits into the dictionary
    X['xia'] = xia
    X['yia'] = yia
    X['zia'] = zia
    X['hcmaxa'] = np.nanmax(xia*1000)  # Convert to mT for plotting
    X['himina'] = np.nanmin(yia*1000)  # Convert to mT for plotting
