        ...
        try:
            ...
            for root, dirs, files in os.walk(path):
                for f in files:
                    ...
                    if f.lower().startswith(input_name.lower()) and f.lower().endswith('.frc'):
                        gen_files.append(os.path.join(root, f))
                        names.append(f)
            if not names:
                print(f"No .frc (FORC) files found for sample '{input_name}'.")
            else:
            ...
            results_filename = "paleo_results_frc.dat"
...
def plot_sample_FORC(x, y, z, SF, sample_name, hcmaxa, min_y, max_y):
    """
    ...
    if (lim_1 != 'K') or (X['sample_copy'] < 2):    
        ...
        while (quest_res == 0):
            ...
            if (quest == 'Y'):
                ...
                plot_sample_FORC(X['Hc'], X['Hu'], X['rho_n'], X['SF'], X['name'],X['hcmaxa'], X['min_y'], X['max_y'])
